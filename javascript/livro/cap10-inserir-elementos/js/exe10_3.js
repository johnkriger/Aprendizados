...
const gravarFilme = (titulo, genero) => {
    ...
    if(localStorage.getItem('filmesTitulo')) {
        ...
        //senão, é a primeira inclusão (salva sem o delimitador)
...
tbFilmes.addEventListener('click', (e) => {
    ...
    if(e.target.classList.contains('exclui')) {
        ...
        if(confirm(`Confirma exclusão do filme "${titulo}"?`)){
            //remove a linha da tabela, correspondente ao simbolo de excuir clicado
            e.target.parentElement.parentElement.remove()

            localStorage.removeItem('filmesTitulo') //exclui filmes salvos em localStorage
            localStorage.removeItem('filmesGenero')

            //salva (somente se existir), acessando o conteúdo da tabela
